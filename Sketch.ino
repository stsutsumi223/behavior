...
const int stimPin[] = {30, 30, 32, 30, 34, 35, 36, 37};
const int rewardPin[] = {4, 4};
...
void configTrial() {
  ...
  tTransmitResults.set(0, 1, &txResults);
  tCueOn.set(0, 0, &cueOn);
  tCueOff.set(500, 0, &cueOff);
  ...
  taskManager.addTask(tTransmitResults);
...
void processResponse(int responseNum) {
  ...
  if (timeResponded > prevTimeResponded) {
    ...
    else if ((timeResponded >= responseWindowStartTime) & (timeResponded <= responseWindowStopTime)) {
      if (!responded) {
        firstResponse = responseNum;
        responded = true;
        if (responseNum == responseRequired) {  // correct choice
          if ((!rewarded) && !(incorrect)) {
            tRewardOn.enable();
            tTransmitResults.enable(); //txResults();
          }
        }
...
void stimOn() {
  ...
  // Photostimulation (20/06/17 ST edited)
  // tRewardRemovalOn.enable();
